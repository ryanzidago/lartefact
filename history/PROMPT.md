# Entity History Artefact Compiler — Prompt

You are compiling a structured knowledge artefact about the history and sociotechnical context of a single entity from a codebase. The artefact will be consumed by LLMs and humans for tasks like assigning reviewers, understanding risk, planning refactors, and making informed decisions about code ownership.
...
- **Entity artefact path** (optional) — path to an existing entity artefact for this entity

## Instructions

If an entity artefact path is provided, use its Key Modules and file paths as your starting point for identifying relevant files. You may discover additional files if the history reveals them (e.g. renamed or deleted files).

If no entity artefact is provided, identify all files in the codebase that are relevant to the named entity — schema definitions, context modules, validation modules, controllers, resolvers, test files, migration files, etc. Search broadly by entity name across filenames, module names, and file contents.
...
Who has contributed the most to this entity's code, by volume and by frequency. Include:

- Author name
- Approximate share of commits and/or lines
- What parts of the entity they focused on (e.g. schema, validations, API, tests)
- Whether they are still active (recent commits in the last 6 months)

### Recommended Reviewers

Who has the most review-relevant knowledge of this entity. Consider:
...
Is knowledge of this entity concentrated in one or two people? Could the team be blocked if they were unavailable?

## Change Patterns

### Change Frequency

How often does this entity change? Characterize as: stable, moderate churn, high churn. Support with data (commits per month/quarter over the entity's lifetime).

### Change Trajectory

Is the entity stabilizing (fewer changes over time) or actively evolving (recent burst of activity)? Include a rough timeline of activity.

### Hotspots

Which files or areas within the entity change most frequently? These are the highest-risk areas for bugs and merge conflicts.

## Evolution

How has this entity changed over its lifetime and why? Tell the story:
...
Evidence of bug fixes, hotfixes, or reverts related to this entity. Look for:
...
- Growing file sizes over time (entity becoming a "god module")
...
1. **Start broad, narrow down** — Search for the entity name across the entire codebase first, then focus historical analysis on the files you find.
2. **Weigh commits intelligently** — A developer who touched 200 files in a bulk refactor is not an "owner" of all 200 entities. Prioritize focused, entity-specific commits over sweeping changes.
